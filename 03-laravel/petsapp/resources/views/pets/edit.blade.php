...
<form method="post" action="{{ route('pets.update', $pet->id) }}" enctype="multipart/form-data" class="pt-6 pb-20">
    @csrf
    @method('PUT')
    <fieldset class="fieldset w-md bg-base-200 border border-base-300 p-4 rounded-box">
        ...
        <div class="avatar mx-auto flex-col gap-2 items-center">
            <div id="upload" class="mask mask-squircle cursor-pointer hover:scale-110 transition-transform">
                <img id="preview" src="{{ asset('images/' . ($pet->image ?? 'no-photo.jpg')) }}" class="max-w-[230px] max-h-[230px] object-cover" />
            </div>
        ...
        <input type="text" name="breed" class="input rounded-full w-full" placeholder="Labrador" value="{{ old('breed', $pet->breed) }}" />

        <label class="fieldset-label">Description</label>
        <textarea name="description" class="textarea rounded-box w-full" placeholder="Description...">{{ old('description', $pet->description) }}</textarea>

        <label class="fieldset-label">Location</label>
        <input type="text" name="location" class="input rounded-full w-full" placeholder="Bogotá" value="{{ old('location', $pet->location) }}" />



        <label class="fieldset-label">Weight (kg)</label>
        <input type="number" min="0" name="weight" class="input rounded-full w-full" placeholder="Ej: 12" value="{{ old('weight', $pet->weight) }}" />

        <label class="fieldset-label">Age (años)</label>
        <input type="number" min="0" name="age" class="input rounded-full w-full" placeholder="Ej: 3" value="{{ old('age', $pet->age) }}" />
